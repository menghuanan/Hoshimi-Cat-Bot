...
- `ModuleAuthor.following` 通过专用 JSON 反序列化器兼容 `true`/`false`/`1`/`0`/`null` 和字段缺失，统一为 `1`/`0`/`null`；其他类型或数字状态降级为 `null`，不得因无业务用途的字段异常丢弃整条动态。
...
- 修改动态作者关注状态解码时，运行 `DynamicListDecodeTest`，并覆盖布尔、`0/1`、`null`、缺失和异常值输入。
