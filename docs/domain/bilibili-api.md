...
新版二维码轮询成功响应的 `url` 可能只包含一次性 `ticket`，不再直接携带 Cookie。检测到该格式后，必须在成功轮询的同一处理路径中立即访问兑换链接，并从重定向链的 `Set-Cookie` 中提取 `SESSDATA`、`bili_jct` 和可选的 `DedeUserID`；旧版直接携带 Cookie 参数的 URL 仍需兼容。ticket 兑换应继续经 `BiliClient` 发起，避免引入独立 HTTP 客户端。
