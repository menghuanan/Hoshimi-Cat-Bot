...
**约束**：动态、开播和下播采用 `DeliveryCoordinator` 联系人级账本；发现阶段必须同时持久化构建输入，构建 channel 与发送 channel 都使用可过期租约恢复。channel 入队与消息构建均不代表成功，只有 `SendTasker` 的平台成功回执可进入 `DELIVERED`，构建与发送失败共享 24 小时/6 次预算。已成功开播的联系人才能建立下播配对，下播成功后才闭合配对；同一动态全部联系人终态后才写入兼容历史。
