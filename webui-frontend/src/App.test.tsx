...
describe('webui shell routing', () => {
  ...
  it('renders subscription filters, create modes, and nested editor controls', async () => {
    ...
    const editorBody = document.querySelector('[data-subscription-editor-body]')
    ...
    expect(editorBody).toHaveClass('lg:grid-cols-[15rem_minmax(0,1fr)]')
    expect(editorBody).not.toHaveClass('lg:justify-center')
    expect(editorPanel).not.toBeNull()
    const editorDialog = screen.getByRole('dialog', {name: '编辑订阅配置'})
    expect(editorDialog).toHaveClass('overflow-hidden')
    expect(editorDialog).not.toHaveClass('lg:grid-rows-[auto_minmax(0,1fr)]')
    expect(editorPanel).not.toHaveClass('lg:row-start-2')
    expect(editorPanel).toHaveClass('overflow-y-auto')
    expect(editorPanel).toHaveClass('w-full')
    expect(editorPanel).not.toHaveClass('lg:max-w-[22rem]')
    expect(screen.getByText('订阅卡片编辑')).toBeInTheDocument()
    expect(screen.getByRole('button', {name: '关闭编辑订阅配置'})).toBeInTheDocument()
